...
const getMyListings = async (req, res) => {
  try {
              ...
              COALESCE(
                json_agg(
                  jsonb_build_object(
                    ...
                    'bidder_info', jsonb_build_object(
                      'user_id', u.user_id,
                      'username', u.username,
                      'email', u.email
                    )
                  )
                ) FILTER (WHERE b.bidder_id IS NOT NULL), '[]'
              ) AS bidders
       FROM listings l
    ...
    res.status(200).json(
      new ApiResponse(200, result.rows, 'Fetched my listings successfully')
    );
  } catch (err) {
    console.error('getMyListings error:', err);
    res.status(500).json(
      new ApiResponse(500, null, 'Failed to fetch my listings')
    );
  }
};
