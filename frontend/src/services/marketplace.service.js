...
export const marketplaceService = {
   ...
   * Fetches seller's listings with bidders information
   * @returns {Promise<Object>} List of user's listings with bidders
   */
  async getMyListings() {
    try {
      ...
      try {
        ...
        // Fallback: get all listings and filter by current user
        // This is a fallback approach when the endpoint is not available yet
        console.warn('my-listings endpoint not available, using fallback');
  ...
  /**
   * Get bidders/enquiries for a specific listing
   * API Endpoint: GET /marketplace/listings/:listingId/bidders
   * @param {string} listingId - Listing ID
   * @returns {Promise<Object>} Bidders and enquiry details
   */
  async getListingBidders(listingId) {
    try {
      const response = await apiClient.get(
        `/marketplace/listings/${listingId}/bidders`
      );
      return response;
    } catch (error) {
      console.error('Error fetching listing bidders:', error);
      throw error;
    }
  },
};
